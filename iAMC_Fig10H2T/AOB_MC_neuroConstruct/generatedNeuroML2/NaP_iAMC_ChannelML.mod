TITLE Mod file for component: Component(id=NaP_iAMC_ChannelML type=ionChannelHH)
...
PARAMETER {
    ...
    m_instances = 3 
    m_steadyState_rate = 0.49962202 
    m_steadyState_midpoint = -59 (mV)
    m_steadyState_scale = 4.9 (mV)
    m_timeCourse_TIME_SCALE = 1 (ms)
    m_timeCourse_VOLT_SCALE = 1 (mV)
    h_instances = 1 
    h_steadyState_rate = 0.49962202 
    h_steadyState_midpoint = -59 (mV)
    h_steadyState_scale = -4.9 (mV)
    h_timeCourse_TIME_SCALE = 1 (ms)
    h_timeCourse_VOLT_SCALE = 1 (mV)
    n_instances = 1 
    n_steadyState_rate = 0.49962202 
    n_steadyState_midpoint = -59 (mV)
    n_steadyState_scale = -4.9 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=NaP_iAMC_ChannelML type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=NaP_iAMC_ChannelML type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
    ? multiply applied to all instances of fcond in: <gates> ([Component(id=m type=gateHHtauInf), Component(id=h type=gateHHtauInf), Component(id=n type=gateHHtauInf)]))
    fopen0 = m_fcond * h_fcond * n_fcond ? path based
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = ((1+(4 * (exp(0 - (( m_timeCourse_V  + 50)/20)^2))))) *  m_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=NaP_iAMC_ChannelML_m_tau_tau)
    ...
    h_timeCourse_t = ((5000+(16000 * (exp(0 - (( h_timeCourse_V  + 50)/20)^2))))) *  h_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=NaP_iAMC_ChannelML_h_tau_tau)
    ...
    n_timeCourse_t = ((2+(4 * (exp(0 - (( n_timeCourse_V  + 50)/20)^2))))) *  n_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=n type=gateHHtauInf), from timeCourse; Component(id=null type=NaP_iAMC_ChannelML_n_tau_tau)
