TITLE Mod file for component: Component(id=KA_iAMC_ChannelML type=ionChannelHH)
...
PARAMETER {
    ...
    h_steadyState_midpoint = -25 (mV)
    h_steadyState_scale = -4.4 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=KA_iAMC_ChannelML type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=KA_iAMC_ChannelML type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    if (m_steadyState_V   < ( -50 ))  { 
        m_steadyState_x = 0 ? evaluable cdv
    } else  { 
        m_steadyState_x = 1 / (1 + exp(0 - ( m_steadyState_V  + 25.7)/4.4)) ? evaluable cdv
    }
    
    m_timeCourse_V = v /  m_timeCourse_VOLT_SCALE ? evaluable
    m_timeCourse_t = ((1+4*exp(-(( m_timeCourse_V -32)/50)^2))) *  m_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=m type=gateHHtauInf), from steadyState; Component(id=null type=KA_iAMC_ChannelML_m_inf_inf)
    m_inf = m_steadyState_x ? path based
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=KA_iAMC_ChannelML_m_tau_tau)
    ...
    h_timeCourse_t = ((1+100*exp(-(( h_timeCourse_V -10)/40)^2))) *  h_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=KA_iAMC_ChannelML_h_tau_tau)
