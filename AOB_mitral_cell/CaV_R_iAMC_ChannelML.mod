TITLE Mod file for component: Component(id=CaV_R_iAMC_ChannelML type=ionChannelHH)
...
PARAMETER {
    ...
    m_steadyState_midpoint = -38.037 (mV)
    m_steadyState_scale = 2.0914001 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=CaV_R_iAMC_ChannelML type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=CaV_R_iAMC_ChannelML type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    if (m_timeCourse_V   < ( -30 ))  { 
        m_timeCourse_t = 28.4118  *  m_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
        m_timeCourse_t = ( 3.1738 + (25.238 * (exp(-1 * (( m_timeCourse_V  + 30)/17.498))))) *  m_timeCourse_TIME_SCALE ? evaluable cdv
    }
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=CaV_R_iAMC_ChannelML_m_tau_tau)
    ...
    h_steadyState_x = ((1/(1+(exp(-1 * ( h_steadyState_V -(-38.037))/-2.0914)))) + (0.6928/(1+(exp(-1 * ( h_steadyState_V -(-38.037))/2.0914))))) ? evaluable
    h_timeCourse_V = v /  h_timeCourse_VOLT_SCALE ? evaluable
    if (h_timeCourse_V   < ( -30 ))  { 
        h_timeCourse_t = 21.063814  *  h_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
        h_timeCourse_t = ( 10.8 + (3.0 * (exp(-1 * (( h_timeCourse_V +20)/8.13))))) *  h_timeCourse_TIME_SCALE ? evaluable cdv
    }
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=h type=gateHHtauInf), from steadyState; Component(id=null type=CaV_R_iAMC_ChannelML_h_inf_inf)
    h_inf = h_steadyState_x ? path based
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=CaV_R_iAMC_ChannelML_h_tau_tau)
